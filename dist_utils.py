...
from set_distance import chamfer, hausdorff
...
class ChamferDist(nn.Module):
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        Args:
            ...
            batch_avg: (bool, optional): whether to avg over batch dim
        """
        adv_pc = adv_pc.unsqueeze(0).transpose(1, 2)
        ori_pc = ori_pc.unsqueeze(0).transpose(1, 2)
        B = adv_pc.shape[0]
        if weights is None:
            weights = torch.ones((B,))
        loss1, loss2 = chamfer(adv_pc, ori_pc)  # [B], adv2ori, ori2adv
        
#        print('loss1.size:',loss1.size(),loss2.size())
