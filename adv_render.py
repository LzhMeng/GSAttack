...
import torch
import math
from gsattack_rasterization import GaussianRasterizationSettings, GaussianRasterizer
from scene.gaussian_model import GaussianModel
from utils.sh_utils import eval_sh
# 这段代码是一个用于渲染场景的函数，主要是通过将高斯分布的点投影到2D屏幕上来生成渲染图像。
def adv_render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    """
    Render the scene. 
    ...
    if pipe.compute_cov3D_python:
        ...
        scales = pc.get_scaling#获取缩放和旋转信息。(对应的就是3D高斯的协方差矩阵了)
    ...
    rendered_image, radii, means2D, points_contribute = rasterizer(
        means3D = means3D,
        means2D = means2D,
        shs = shs,
    ...
    return {"render": rendered_image,
            ...
            "radii": radii,
            "means2D": means2D,
            "points_contribute":points_contribute}
